...
from sbm_agent_factory.validate import clean_files
def test_prohibited(tmp_path):
    (tmp_path/'.sbm').mkdir(); (tmp_path/'.sbm/scaffold.json').write_text('{}')
    try: clean_files(tmp_path); assert False
    except FactoryError as e: assert e.code=='PROHIBITED_ARTIFACT'
