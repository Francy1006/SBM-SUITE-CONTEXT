...
def test_draft_blocked(tmp_path):
    b=write_fixture_bundle(tmp_path/'i',TEMPLATE,agent_id='D'); p=yaml.safe_load(b['proposal'].read_text()); p['review_status']='DRAFT'; b['proposal'].write_text(yaml.safe_dump(p,sort_keys=False))
    try: create_agent(*args(b),TEMPLATE,tmp_path/'o',CFG); assert False
    except FactoryError as e: assert e.code=='PROPOSAL_NOT_APPROVED' and e.status=='BLOCKED'
