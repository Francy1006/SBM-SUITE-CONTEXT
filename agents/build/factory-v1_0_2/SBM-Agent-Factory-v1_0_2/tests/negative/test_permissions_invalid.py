...
from sbm_agent_factory.permissions import validate_permissions
def test_permissions_conflict():
    try: validate_permissions({'authority':['READ']},{'allowed_actions':['READ'],'denied_actions':['READ']}); assert False
    except FactoryError as e: assert e.code=='PERMISSION_CONFLICT'
