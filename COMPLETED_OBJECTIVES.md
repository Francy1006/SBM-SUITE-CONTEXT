...
| OBJ-CTX-042 | SBM-SUITE | Integrar Documentation Markdown con Notion mediante sincronización Git→Notion controlada, preservando Git/Markdown como source of truth, estructura de páginas, IDs estables, trazabilidad y detección de cambios; bidireccionalidad queda fuera del alcance inicial. | completed | 5 | FEATURE-syncs-documentation-to-notion | N/A | 2026-09-13 | Implemented controlled Git→Notion publication from the Documentation upgrade lifecycle through SBM-UTIL, with bounded HTTP configuration and pending retry state while preserving Markdown as source of truth. | Canonical full-suite QA passed: Context QA passed; the with-Sonar queue passed for DP-API, KS-STORE, sbm-ai-assistant, SBM-API, SBM-DB, SBM-MANAGER and SBM-UTIL; 7 executed, 0 skipped. | N/A | `feat(context): finalize Notion sync and SBM-UTIL` |
| OBJ-CTX-005 | SBM-SUITE | Crear SBM-UTIL como servicio reutilizable Java/Spring Boot para email, archivos, APIs externas, conectores determinísticos, transformaciones técnicas y tipos de cambio oficiales consumidos por servicios/agentes. | completed | 5 | FEATURE-syncs-documentation-to-notion | N/A | 2026-09-13 | Established SBM-UTIL as a reusable suite utility/integration service; the evidenced repository contains a Java application, service-token security and the Notion documentation synchronization implementation. | Canonical full-suite QA passed: Context QA passed; the with-Sonar queue passed for DP-API, KS-STORE, sbm-ai-assistant, SBM-API, SBM-DB, SBM-MANAGER and SBM-UTIL; 7 executed, 0 skipped. | N/A | `feat(context): finalize Notion sync and SBM-UTIL` |
