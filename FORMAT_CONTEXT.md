...
46. `objective-activation` is the only lifecycle phase for existing objective transitions from `pending` to `active`. It requires a non-empty array of unique full objective items whose desired `status` is `active`.
47. During `objective-activation`, preserve `objective_id`, `objective`, `priority`, `target_date`, project identity and documentation literally from the existing pending row. The explicit validated manifest `branch` is the desired branch and may differ from the current pending branch; reject missing, already-active, completed, duplicated or otherwise invalid objective IDs.
48. `objective-activation` must validate the complete batch before generating output, replace the complete applicable Active objectives and Pending objectives sections, remove exactly the selected pending rows without leaving an internal table gap and insert every selected active row as one contiguous block immediately after the last active data row and before the first blank line, `Rules:` block or other non-table content. It must preserve unrelated rows and trailing prose byte-for-byte, never emit a partial batch, never reuse creation/insertion-only behavior and never append completed history.
49. Every lifecycle route accepts an atomic array of 1..N unique objectives. A batch may span multiple registered projects when each item carries its literal project identity and the operation is routed through `sbm-suite-context`; validation of every item and target precedes every generated/applied change.
50. `planning-activation` may create each new objective directly as `pending`, `active` or `completed`. Direct completion writes exactly one terminal-history row and never inserts that objective into an operational table. Fast-track creation never bypasses full-suite QA or Documentation.
51. Every lifecycle batch requires structured `passed` evidence from `QA/qa-full.sh`. Full QA comprises Context QA and the sequential all-project with-Sonar queue; `not-applicable` is not an authorizing lifecycle result.
52. Transversal Git work starts from `main`, merges with `--no-ff` directly into `main`, checks out/pushes `main`, then deletes the temporary branch locally/remotely. Finalization requires current Context, implementation, QA and Documentation gates for the exact ordered objective batch and shared branch.
...
- `objective-activation` accepts one or more unique objectives already present exactly once in Pending objectives and requires every complete desired item to use `status=active`.
- During objective activation, preserve ID, objective, priority, target date, project and documentation literally for every item; move only the selected rows from Pending objectives to Active objectives, change status from `pending` to `active`, and apply each explicit valid desired branch even when it differs from the pending branch.
...
- Insert the activated row inside the continuous Active objectives table before its first blank line, `Rules:` block or other prose; remove the pending row without introducing a blank line, and preserve every unrelated row byte-for-byte in its original order.
- Update only the selected project's row in `## 6. Project objective summaries`, changing only cells strictly affected by the activation and preserving all other summary rows byte-for-byte.
...
- Append only newly terminal objectives (`completed`, `registered`, `cancelled`, `deleted`), including direct-created `completed` items.
...
- Allowed final statuses: `completed`, `registered`, `cancelled`, `deleted`.
- Every temporary branch that creates or changes one or more objective lifecycle records must pass canonical full-suite QA after Context/implementation changes and before Documentation/Git finalization. Fast-track lifecycle transitions may skip intermediate statuses but never the QA gate.
...
- A lifecycle-only or no-op objective may close with an empty Git diff when the objective exists in the current operational context, the requested lifecycle phase is `implementation-closure`, full-suite QA is `passed`, and no unsupported implementation claim is introduced.
- `implementation-closure` must also remove every selected objective from the applicable operational objective sections and update the applicable QA contexts in the same upgrade. Direct creation as `completed` is a Context mutation and is validated later by the mandatory branch QA gate before Documentation/finalization.
- `patches/completed-objectives.json` is allowed for `implementation-closure`, `objective-registration`, `objective-completion`, `objective-deletion`, and `planning-activation` only when at least one requested item is created directly as `completed`.
...
- Resolve every selected objective's canonical project grouping heading from the Project Registry/objective project identity; for `dp-api`, it is `### DP-API`. A multiproject batch may therefore touch several project groups in the same single operation.
- If none of the selected canonical project headings exists outside fenced code blocks, `append_to_section` may append all missing selected project groups, each with the exact required table header and every selected terminal row for that project.
- If any selected canonical project heading already exists, use `replace_section` and return the complete current `## 1. Completed objectives by project` section, preserving all preamble text, project headings, tables and unrelated rows while adding every selected terminal row and any missing selected project groups.
- Never use `append_to_section` when any selected canonical project heading already exists.
- Reject multiple matches for a canonical project heading, duplicate Objective IDs and duplicate project grouping headings.
...
- `planning-activation` creates new objectives only and may place them directly in pending, active or completed state; it never transitions an existing row.
- `objective-activation` transitions one or more existing pending objectives atomically, preserving each ID, objective, priority, target date and documentation literally, changing status and applying each explicit valid desired branch, which may differ from the current pending branch.
...
63. Require `planning-activation` to validate the complete 1..N batch atomically before producing any patch. New `pending`/`active` items are written to operational objective sections; items created directly as `completed` are written only to global `COMPLETED_OBJECTIVES.md`. For a single project-scoped target, synchronize operational items across project/global Context; for an explicitly multiproject batch routed through `sbm-suite-context`, the global `PROJECT_CONTEXT.md` is the transaction authority for operational items. Reject IDs that already exist in any lifecycle state.
...
67. Accept only `planning-activation`, `objective-activation`, `objective-registration`, `objective-completion`, `objective-deletion`, `objective-update`, `implementation-progress` and `implementation-closure` as `lifecycle_phase` values.
68. Never infer `lifecycle_phase` from `qa-results.md`, `git-diff.patch`, `changed-files.txt`, test status or RAG context.
69. Require `objectives` to be a non-empty 1..N array with unique valid `objective_id` values for every lifecycle. For creation, activation, registration, completion, deletion and update, require the full lifecycle fields, preserve literal project/documentation identity, and validate the complete possibly multiproject batch before producing any patch. Allow planning creation directly as `pending`, `active` or `completed`; allow `objective-update` only between operational `pending` and `active` states.
70. Do not derive `execution_mode` from `lifecycle_phase`. `planning-activation` and `objective-activation` may run in `evidence` or `user-guided` mode. Require `USER_PROMPT.md` only when `execution_mode=user-guided`, forbid it when `execution_mode=evidence`, and never synthesize `USER_PROMPT.md` from `manifest.objectives[]`. `objective-activation` always forbids `patches/completed-objectives.json`; `planning-activation` requires it iff at least one requested item has `status=completed` and forbids it otherwise.
...
72. Require `implementation-closure` to include `patches/completed-objectives.json`, `patches/global-project-context.json` and `patches/global-qa-context.json` for every target. Project-scoped targets additionally require `patches/project-context.json` and `patches/project-qa-context.json`; `sbm-suite-context` forbids those project-scoped patches. Require executed canonical full-suite `passed` evidence; `not-applicable`, missing or failed evidence never authorizes lifecycle finalization. Require explicit closure and copy the complete tooling-generated `qa` object, including the SHA-256 of the exact QA evidence, literally through source and output manifests. Require implementation evidence only when implementation changes are claimed; allow lifecycle-only or no-op closure with an empty Git diff when the objective exists in the current operational context and no implementation claim is generated.
73. During a terminal transition, remove only the selected objective IDs, preserve every other objective and append every selected ID exactly once to `COMPLETED_OBJECTIVES.md` with its requested final status.
74. Require every closure QA patch to use explicit successful `qa-results.md` evidence. Missing, empty, invalid, `not-applicable` or failed evidence blocks closure. Preserve every unrelated project summary, test row and current QA record. Project-scoped targets require both project/global QA patches; `sbm-suite-context` requires only the global QA patch.
...
78. Allow `append_to_section` for `patches/completed-objectives.json` only when none of the selected canonical project headings exists; also allow it for `## 18. Historical decisions` in global `PROJECT_CONTEXT.md` and `## 22. Historical decisions` in project `PROJECT_CONTEXT.md`.
...
81. When none of the selected canonical project headings exists during a terminal/direct-completed batch, allow one `append_to_section` operation containing every missing selected project group, the exact required table header and all selected rows grouped by project.
82. When any selected canonical project heading exists exactly once, require one `replace_section` operation with the complete current history section, preserving every existing project heading and historical row while adding all selected rows under existing headings and appending any missing selected project groups.
83. Never allow `append_to_section` when any selected canonical project heading exists; reject multiple canonical project heading matches and duplicate project headings.
...
93. Require `objective-activation` to prove every selected ID is unique, exists exactly once with current `status=pending` in every applicable complete operational context and is absent from completed history; reject the entire batch when any item fails.
94. During `objective-activation`, require every `objective_id`, `objective`, `priority` and `target_date` to equal its existing pending row literally, require desired `status=active`, and require an explicit valid desired `branch` for every item, allowing that branch to differ from the current pending row and allowing multiple items to share one branch.
95. Require `objective-activation` to replace both complete operational lifecycle sections, remove only the selected pending rows without leaving an internal blank line, add every selected active row as one contiguous block inside the table before the first blank line or `Rules:` block, preserve all unrelated rows and trailing prose byte-for-byte, update only the selected project's strictly affected summary row with every activated ID and the shared branch when applicable, and never create, duplicate or partially activate an objective batch.
...
```json
{
  ...
  "lifecycle_phase": "<planning-activation|objective-activation|objective-registration|objective-completion|objective-deletion|objective-update|implementation-progress|implementation-closure>",
  "objectives": [
    {
