...
- Every temporary branch starts from synchronized `main` and merges with
  `--no-ff` directly into `main`.
- Every objective batch requires complete suite QA and updated Documentation,
  including lifecycle-only and fast-track changes.
- Successful finalization returns every repository to `main` and removes the
  temporary branch locally/remotely.
